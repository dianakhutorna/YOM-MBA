...
SCRIPT_CONFIG_PATH = Path("training/configs/train_ranker_with_pop_features.yaml")
...
def main():
    config_path, features_path = parse_config_args(
        ...
        description="Train LightGBM ranker with popularity features",
    )
    cfg = load_yaml_config(config_path) if config_path.exists() else {}
    global SPLIT_DATE, MIN_COOC, MIN_LIFT, K_CANDIDATES, K_EVAL, TRAIN_KIOSK_RATIO, MAX_NEG_PER_GROUP, FEATURE_COLS
    ...
    if "feature_cols_base" in cfg:
        FEATURE_COLS = list(cfg["feature_cols_base"])
    setup_logging()
